...
const MobileLandingScreen: React.FC<MobileLandingScreenProps> = ({ onAuthSuccess }) => {
  const { login, isPolling } = useMobileAuth();
  ...
  const handleLogin = async () => {
    try {
      ...
      console.log('🚀 [MOBILE LANDING] Starting mobile CAS authentication');
      
      const result = await login();
      
      if (result.success) {
        console.log('✅ [MOBILE LANDING] Login initiated successfully');
        // The polling will handle the rest
      } else {
        console.log('❌ [MOBILE LANDING] Login failed:', result.message);
        Alert.alert('Login Failed', result.message);
      }
    } catch (error) {
      console.error('💥 [MOBILE LANDING] Error during login:', error);
